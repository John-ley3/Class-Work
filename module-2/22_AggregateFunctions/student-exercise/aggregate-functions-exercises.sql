...
-- name. 
-- (19 rows)
SELECT population, (name + ', ' + district) AS name_and_state 
FROM city
...
SELECT (gnp * 1000000) / population AS percapita 
FROM country 
...
SELECT Avg(lifeexpectancy) AS avglifeexpectancy FROM country WHERE continent='South America';
...
SELECT Max(population) AS citymaxpopulation FROM city;

-- 9. The maximum population of all cities in Australia.
-- (largest city population in Australia: 3276207)
SELECT Max(population) AS maxpopulation FROM city WHERE countrycode='AUS';
...
SELECT Avg(population) AS usa_avg_pop
FROM city 
WHERE countrycode='USA';

-- 12. The average population of cities in China.
-- (average city population in China: 484720.6997 approx.)
SELECT Avg(population) AS chn_avg_population
FROM city
...
SELECT Top 10 lifeexpectancy, (population / surfacearea) AS populationdensity, name 
FROM country
...
SELECT ABS(gnp - gnpold) AS absognpdifference,  (gnp - gnpold) AS gnpdifference, name
FROM country
WHERE gnp Is Not Null AND gnpold Is Not Null
ORDER BY absognpdifference;
...
-- (45 rows)
SELECT district, count(name) AS citycount
FROM city
WHERE countrycode ='USA'
GROUP BY district
ORDER BY district;
...
SELECT max(population) AS maxpopulation, countrycode
FROM city
GROUP BY countrycode
