<template>
  <div class="main">
    ...
    <div class="well-display">
      <div class="well" @click="filter=0">
        <span class="amount">{{ averageRating }}</span>
        Average Rating
      </div>

      <div class="well" @click="filter=1">
        <span class="amount">{{ numberOfOneStarReviews }}</span>
        1 Star Review{{ numberOfOneStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="filter=2">
        <span class="amount">{{ numberOfTwoStarReviews }}</span>
        2 Star Review{{ numberOfTwoStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="filter=3">
        <span class="amount">{{ numberOfThreeStarReviews }}</span>
        3 Star Review{{ numberOfThreeStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="filter=4">
        <span class="amount">{{ numberOfFourStarReviews }}</span>
        4 Star Review{{ numberOfFourStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="filter=5">
        <span class="amount">{{ numberOfFiveStarReviews }}</span>
        5 Star Review{{ numberOfFiveStarReviews === 1 ? '' : 's' }}
      </div>
    </div>
    <a href="#" v-on:click.prevent="showForm = true" v-show="!showForm">Show Form</a>
    <form v-show='showForm'>
      ...
      <div class="form-element">
        ...
        <textarea type='text' id="review" v-model="newReview.review"/>
      </div>
      <div class="form-element">
      ...
      <input type='button' value="Cancel" v-on:click.prevent="resetForm();showForm=false;"/>
    </form>
    <div
      class="review"
  ...
  computed: {
    filteredReviews() {
        return this.reviews.filter((review) => {
          return this.filter === 0 ? true : review.rating == this.filter;
        })
    },
    averageRating() {
