    ...
    <table id="tblUsers">
      ...
      <tbody>
        ...
        <tr v-for="user in filteredList" v-bind:key="user.id" v-bind:class="{ disabled: user.status === 'Disabled' }">
          ...
          <td>{{ user.status }}</td>
          <td>
            <button v-on:click="flipStatus(user.id)"> {{ user.status === 'Active' ? 'Disable' : 'Activate' }} </button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    flipStatus(userID) {
      this.users[userID - 1].status = this.users[userID - 1].status === 'Active' ? 'Disabled' : 'Active';
    },
  },
  computed: {
    filteredList() {
